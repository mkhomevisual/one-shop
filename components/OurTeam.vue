  ...
  <section id="team" class="py-24 sm:py-32 bg-white dark:bg-neutral-900 transition-colors">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      ...
      <ul class="mt-16 flex flex-wrap justify-center gap-8">
        <li
          v-for="p in peopleView" :key="p.email"
          ...
          <!-- Photo -->
          <div
            class="mx-auto mb-6 h-48 w-48 aspect-square rounded-xl
          ...
          <div class="mt-6 space-y-3">
            <!-- Telefon: úplně skrytý (žádná čísla) -->
            <a v-if="p.displayPhone"
               :href="p.telHref"
...
type RoleKey = 'ceo' | 'salesDirector' | 'salesRep' | 'designer' | 'warehouseLead' | 'regionalManager'
type Person = { name: string; roleKey: RoleKey; phone: string; email: string; imageUrl: string }

const people: Person[] = [
  { name: 'Ing. Viet Duc Le',   roleKey: 'ceo',            phone: '+420 000 000 000', email: 'le@voph.cz',      imageUrl: person1 },
  { name: 'Hoang Pham Van',     roleKey: 'salesDirector',  phone: '+420 000 000 000', email: 'nam@voph.cz',     imageUrl: person2 },
  { name: 'Nguyen Van Manh',    roleKey: 'salesRep',       phone: '+420 000 000 000', email: 'manh@voph.cz',    imageUrl: person3 },
  { name: 'Kristýna Podlahová', roleKey: 'designer',       phone: '+420 000 000 000', email: 'podlahova@voph.cz', imageUrl: person4 },
  { name: 'Vít Sechovec',       roleKey: 'warehouseLead',  phone: '+420 000 000 000', email: 'vit@voph.cz',      imageUrl: person5 },
  { name: 'Marcel Rathouský',   roleKey: 'regionalManager',phone: '+420 000 000 000', email: 'marcel@voph.cz',   imageUrl: person5 },
]
...
/** Tel řádek úplně schováme (displayPhone = '' → v-if false) */
const peopleView = computed(() =>
  people.map(p => ({
    ...p,
    role: t.value.roles[p.roleKey],
    displayPhone: '',  // <- žádná čísla, žádný výstup
    telHref: undefined
  }))
)
